...
export function AddWorkerModal({
  ...
  const workTimeFallback = () => ({
    start: form.workStartTime || DEFAULT_WORK_START,
    end: form.workEndTime || DEFAULT_WORK_END,
  });

  // 요일별 모드로 켜는 순간 선택된 근무요일을 현재 단일 시간으로 채운다
  const togglePerDayWorkTime = (enabled: boolean) => {
    if (enabled) {
      onUpdateForm(
        'workTimes',
        fillMissingWorkTimes(
          form.workTimes,
          labelsToSortedNums(form.workDays),
          workTimeFallback(),
        ),
      );
    }
    onUpdateForm('perDayWorkTime', enabled);
  };

  const setWorkTime = (day: WorkDay, field: 'start' | 'end', value: string) => {
    onUpdateForm(
      'workTimes',
      patchWorkTime(form.workTimes, day, field, value, workTimeFallback()),
    );
  };
            ...
            <div className="space-y-3">
              ...
              <div className="mt-3">
                <Checkbox
                  ...
                  checked={form.perDayWorkTime}
                  onChange={(e) => togglePerDayWorkTime(e.target.checked)}
                  className="text-sm"
                />
              </div>
              ...
              {form.perDayWorkTime && (
                <div className="mt-3">
                  <WorkTimeByDayEditor
                    workDays={labelsToSortedNums(form.workDays)}
