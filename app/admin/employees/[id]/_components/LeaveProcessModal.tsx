    ...
    <Modal isOpen={isOpen} onClose={onClose} title="연차 처리" size="sm">
      <div className="space-y-4">
        ...
        <div className="text-sm text-gray-700">
          <p className="mb-2">
            <span className="font-semibold text-gray-900">{dateDisplay}</span> 기록
          </p>
          {isAnnualLeave ? (
            <p>
        ...
        {isAnnualLeave && record.workContent && (
          <div className="rounded-lg bg-gray-50 border border-gray-200 p-3">
            <p className="text-xs text-gray-500 mb-1">연차 사유</p>
            <p className="text-sm text-gray-700 whitespace-pre-wrap">{record.workContent}</p>
          </div>
        )}
