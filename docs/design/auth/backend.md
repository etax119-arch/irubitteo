...
이 문서는 durubitteo 프로젝트의 NestJS 서버(durubitteo_server)에 구현된 인증 시스템을 정리한 문서입니다.
...
import { IsString, IsEnum, IsEmail, IsNotEmpty, Length, ValidateIf } from 'class-validator';
...
export class LoginDto {
  ...
  // Admin용 - role이 admin일 때만 필수
  @ValidateIf((o) => o.role === UserRole.ADMIN)
  @IsNotEmpty({ message: 'Email is required for admin login' })
  @IsEmail()
  email?: string;

  @ValidateIf((o) => o.role === UserRole.ADMIN)
  @IsNotEmpty({ message: 'Password is required for admin login' })
  @IsString()
  password?: string;

  // Company/Employee용 - role이 company 또는 employee일 때만 필수
  @ValidateIf((o) => o.role === UserRole.COMPANY || o.role === UserRole.EMPLOYEE)
  @IsNotEmpty({ message: 'Code is required for company/employee login' })
  @IsString()
  @Length(1, 20)
  code?: string;
}
...
export class AuthController {
  ...
  private setAuthCookies(
    ...
    // Refresh Token - HttpOnly, /v1/auth 경로만
...
- [x] Auth Module 생성 (`src/auth/auth.module.ts`)
- [x] LoginDto 유효성 검증 (`src/auth/dto/login.dto.ts`)
- [x] 역할별 사용자 조회 로직 (`src/auth/auth.service.ts`)
- [x] 관리자 비밀번호 bcrypt 해싱 및 검증
- [x] JWT Access/Refresh Token 발급
- [x] Cookie 설정 (HttpOnly, Secure, SameSite)
- [x] JWT Strategy - Cookie에서 토큰 추출 (`src/auth/strategies/jwt.strategy.ts`)
- [x] JwtAuthGuard, RolesGuard (`src/auth/guards/`)
- [x] /auth/login, /auth/refresh, /auth/logout, /auth/me 엔드포인트

### 보안 구현 ✅

- [x] 비밀번호 해싱 (bcrypt, saltRounds: 12)
- [x] JWT Secret 충분한 길이 (32자 이상)
- [x] 프로덕션 환경에서 Secure 플래그 활성화
- [x] Refresh Token은 /auth 경로로 제한
- [x] 로그인 실패 시 구체적인 정보 노출 금지

### 데코레이터 ✅

- [x] @Public() - 인증 없이 접근 가능 (`src/auth/decorators/public.decorator.ts`)
- [x] @Roles() - 역할 기반 접근 제어 (`src/auth/decorators/roles.decorator.ts`)
- [x] @CurrentUser() - 현재 사용자 정보 주입 (`src/auth/decorators/current-user.decorator.ts`)

### 테스트 항목

- [x] 관리자 로그인 (이메일 + 비밀번호)
- [x] 기업 로그인 (고유번호)
- [x] 직원 로그인 (생년월일4자리+전화번호뒤4자리)
- [x] 토큰 갱신
- [x] 로그아웃 (Cookie 삭제 확인)
- [x] 잘못된 자격 증명 에러 처리
- [x] 만료된 토큰 에러 처리
...
- [프론트엔드 인증](./frontend.md) - Next.js 클라이언트 인증 구현
