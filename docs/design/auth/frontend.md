...
| auth-status | No | Yes | Strict | / | 604800 (7일) | 인증 상태 플래그 ("authenticated") |
| user-role | No | Yes | Strict | / | 604800 (7일) | 사용자 역할 ("admin" / "company" / "employee") |

**서버 응답 예시** (Set-Cookie 헤더):
```
Set-Cookie: accessToken=eyJ...; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=900
Set-Cookie: refreshToken=eyJ...; HttpOnly; Secure; SameSite=Strict; Path=/v1/auth; Max-Age=604800
Set-Cookie: auth-status=authenticated; Secure; SameSite=Strict; Path=/; Max-Age=604800
Set-Cookie: user-role=admin; Secure; SameSite=Strict; Path=/; Max-Age=604800
```
