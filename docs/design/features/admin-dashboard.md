...
- `/lib/api/admin.ts` - Admin API (통계, 출퇴근, 월간 통계, 알림, 파일 관리, 관리자 계정 생성/목록 조회)
...
| content | 콘텐츠 관리 | Image | 갤러리, 뉴스레터 및 공고 관리 |
...
**데이터 관리**: 서버 사이드 검색 + 페이지네이션 (`useAdminDailyAttendance`, 검색 디바운싱 300ms, `PaginationBar`)

**날짜 선택**:
- 이전/다음 날짜 버튼
- 날짜 선택 input (캘린더)
- 회사명 검색창 (서버 사이드 검색, 300ms 디바운싱)

**회사별 아코디언**:
- 회사명, 출근 인원 표시 (예: "3/5명 출근")
- 상태별 집계 배지 (출근 전, 근무중, 결근, 휴무, 퇴근, 휴무)
...
**데이터 관리**: 서버 사이드 페이지네이션 (`useCompanies`, 검색 디바운싱 300ms, `PaginationBar`)

#### 상단 영역
- "회원사 관리" 제목
- 검색창 (회사명 검색, 서버 사이드, 300ms 디바운싱)
- "+ 회원사 추가" 버튼
...
**목적**: 플랫폼 전체 근로자 목록 조회
**데이터 관리**: 서버 사이드 페이지네이션 (`useAdminEmployees`, staleTime: 5분, 검색 디바운싱 300ms, `PaginationBar`, `onNextPage` 상한 검증)

#### 상단 영역
- "근로자 관리" 제목
- 검색창 (이름, 회사명 서버 사이드 검색, 300ms 디바운싱)
- 필터 버튼
...
**데이터 관리**: 서버 사이드 검색 + 페이지네이션 (`useAdminMonthlyStats`, 검색 디바운싱 300ms, `PaginationBar`)

#### 상단 영역
- "근무 통계" 제목
- 검색창 (회사명 서버 사이드 검색, 300ms 디바운싱)
...
- 테이블:
  ...
  - 푸터: 평균 출근 일수, 평균 근무시간
- 하단: 서명란 (기업 대표자, 이루빛터 담당자) + "이루빛터 중앙 통제 시스템"

**프리뷰 모드** (PDF 미리보기):
- `generateWorkStatsPdf()` (jsPDF + autoTable) 호출 → blob URL → iframe 렌더링
- "수정하기" 버튼 → 편집 모드 복귀
- "PDF 다운로드" 버튼 + "인쇄" 버튼
- 담당자 정보도 PDF에 포함
- autoTable 자동 페이지 나눔으로 근로자 수에 관계없이 내용 잘림 없음
...
- 공용 `validateUploadFile()` + `FILE_CONSTRAINTS.REPORT`로 클라이언트 검증
...
- [x] 헤더 우측 계정 설정 버튼 + 계정 설정 페이지 (`/admin/settings`)
- [x] 관리자 계정 추가 폼 (이메일/이름/초기 비밀번호 + 검증)
- [x] 관리자 계정 리스트 섹션 (이름/이메일/생성일시, 로딩/에러/빈 상태)
...
- [x] 관리자 계정 생성 (`POST /admin/accounts`)
- [x] 관리자 계정 목록 조회 (`GET /admin/accounts`)
- [x] 더미 데이터 제거
...
### 개선 예정
- [x] 인쇄 기능 고도화 (jsPDF + autoTable 기반 PDF 생성으로 전환 완료)
...
| 소속 근로자 | 근로자 테이블 (이름, 전화번호, 입사일, 장애유형, 고유번호) + 페이지네이션 (20명/페이지) | - |
...
getEmployees({ companyId: id, page, limit: 20 }) → 소속 근로자 목록 (페이지네이션)
...
- **파일 관리**: 드래그&드롭 또는 클릭 업로드, 파일 조회/다운로드/삭제, 파일 크기 표시, 업로드 시 `validateUploadFile` + `FILE_CONSTRAINTS.DOCUMENT` 검증 (toast 에러 표시)
...
| 모달 | 설명 |
|------|------|
| 근무시간 수정 | 출근/퇴근 시간 수정 + 업무내용 textarea + 상태 드롭다운 (출근/퇴근/결근/휴무) |
...
> (`checkin`=출근, `checkout`=퇴근, `absent`=결근, `leave`=휴무)
...
- **상태 변경**: 근무시간 수정 모달에서 상태 드롭다운으로 출근/퇴근/결근/휴무 선택 가능, 변경 시 프로필 카드 상태 자동 갱신
