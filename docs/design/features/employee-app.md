...
```
/app/employee/
├── layout.tsx              # 직원용 레이아웃 (인증 보호 — useAuth)
├── page.tsx                # 메인 화면 (출근/퇴근 버튼, useAuthStore 사용)
...
**오늘의 작업 내용 섹션**:
- **휴일인 경우**: 빨간 계열 배경 (`bg-red-50 border-red-200`), "오늘은 휴일입니다" 헤더 (빨간 텍스트), 메모 있으면 표시 / 없으면 "휴일입니다." 표시
- **일반 근무일**: 연한 주황색 배경, 기업 관리자가 근무일정관리에서 등록한 작업 내용 표시
- 출근 버튼은 휴일에도 유지 (휴일에도 출근 가능 — 블로킹하지 않음)
- 예시:
  ```
  오늘은 제품 포장 작업과
  부품 조립 작업을 진행할 예정입니다.
...
**출근 시간 설정**:
- 시(0~23) / 분(0~59) 드롭다운 선택
- 시간 선택 필수 (시/분 모두 선택해야 출근 완료 버튼 활성화)
- 선택한 시간은 KST 기준 ISO 8601 문자열로 변환하여 API `clockIn` 필드로 전송

**출근 완료 버튼**:
- 체크박스 선택 + 시/분 모두 선택 시 활성화
- 클릭 시 출근 완료 모달 표시
...
### 5. 활동 기록 (WorkRecordsSection)

**목적**: 과거 출퇴근 기록 및 업무 내용 조회
...
- iPhone HEIC/HEIF 이미지를 브라우저에서 자동 변환 표시
- 이미지 로드 실패(`onError`) 시 HEIC로 간주하고 변환 시도
- `heic-to` 라이브러리로 JPEG 변환
- 변환 실패 시 에러 아이콘 폴백

**사진 확대 모달** (`PhotoLightbox`):
- 전체 화면 오버레이 (읽기 전용 뷰어)
