...
- 휴무: 청록색 (`bg-teal-100 text-teal-700`)
...
- 테이블 컬럼: 날짜, 출근, 퇴근, 상태(출근/퇴근/결근/휴무), 업무내용, 수정
- 상태 표시는 attendance DB status를 그대로 사용:
  - `checkin` → 출근
  - `checkout` → 퇴근
  - `absent` → 결근
  - `leave` → 휴무
- 휴무/결근 상태: 출근/퇴근 시간은 `-`로 표시
- 업무내용: "확인하기" 링크 → 모달로 전체 내용 표시
- 수정: 각 행에 수정 아이콘 → 근무시간 수정 모달
  - 출근시간, 퇴근시간, 업무내용 편집
  - 상태 드롭다운: 출근(`checkin`), 퇴근(`checkout`), 결근(`absent`), 휴무(`leave`) 선택 가능
...
#### 날짜 셀
- 날짜 숫자
- **휴일인 경우**: 빨간 계열 배경 (`bg-red-50 border-red-300`), "휴일" 라벨 (빨간 텍스트), 메모 있으면 표시
- 등록된 일정 있는 경우 (휴일 아닌): 파란 계열 배경, "업무 지시서" 라벨
- 일정 없는 평일: + 아이콘
- 주말: 회색 배경
- 오늘 날짜: 주황색 테두리
- **우선순위**: 휴일 > 주말 > 일반 스케줄 > 빈 셀

#### 일정 등록/수정 모달
- 날짜 클릭 시 모달 표시
- 선택한 날짜 표시
- **"휴일로 설정" 체크박스** (기존 `@/components/ui/Checkbox` 재사용)
  - 체크 시 안내 문구: "휴일에는 출근이 필요하지 않으며, 결근 처리되지 않습니다."
  - 텍스트 영역 라벨 → "메모 (선택사항)", placeholder 변경, rows 축소 (4줄)
  - **content 빈 값 허용** (휴일은 메모 없이 등록 가능)
- 휴일 아닌 경우: 근무 내용 텍스트 영역 (8줄), content 필수
- 기존 일정인 경우: 삭제 버튼 추가
- 취소 / 저장 버튼
...
| 근무 일정 | 캘린더 모달 | 날짜별 작업 내용 또는 휴일 설정 |
...
interface CompanyDailyResponse {
  ...
  isHoliday: boolean;               // 해당 날짜 회사 휴일 여부
