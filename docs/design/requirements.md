...
| 역할 | 권한 |
|------|------|
| 이루빛터 관리자 | 플랫폼 전체 관리, 기업 계정 생성/관리 |
...
### 근로자 흐름
1. 고유번호로 로그인
2. 출근 버튼 클릭 → 오늘의 작업 내용 확인 → 시간 기록
3. 퇴근 버튼 클릭 → 업무 내용 입력 (필수) + 사진 첨부 (선택) → 시간 기록
4. 본인 출퇴근 이력 조회
...
### 직원 로그인
- **로그인 ID**: 주민등록번호 MMDD (3~6번째 자리) + 전화번호 뒤 4자리 (8자리)
  - 예: `03155678` (주민번호 `990315-...` → `0315`, 전화번호 `010-...-5678` → `5678`)
- 기업이 근로자 등록 시 자동 생성 (수동 수정 가능)
- 비밀번호 없음
...
| 이름 | O | 실명 |
| 고유번호 | O | 로그인용 (자동 생성) |
| 연락처 | O | 휴대폰 번호 (하이픈 포함 저장, 예: 010-1234-5678) |
| 성별 | O | male / female |
| 주민등록번호 | O | 평문 저장 |
| 입사일 | O | YYYY-MM-DD |
| 퇴사일 | X | 퇴사 시 입력 |
...
| 근무 시작 시간 | O | 기본 09:00 |
| 첨부 파일 | X | 계약서 등 (문서 관리 기능) |
| 장애 유형 | O | 지체장애, 시각장애 등 (15개 유형) |
| 장애 중증도 | O | 중증 / 경증 |
| 장애인 인정일 | O | YYYY-MM-DD |
| 주소 (시/도) | O | 17개 시/도 중 선택 |
| 주소 (시/군/구) | O | 시/도에 따라 동적 목록 |
| 상세주소 | X | 자유 입력 (최대 200자) |
| 비상연락처 이름 | O | 보호자/가족 이름 |
| 비상연락처 관계 | O | 부모, 배우자 등 |
| 비상연락처 전화번호 | O | 휴대폰 번호 |
